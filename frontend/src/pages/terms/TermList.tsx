...
class TermListingList extends React.Component<Props, State> {
  ...
  render() {
    ...
    let { terms, goToTermListing, context, showActionDialog, stats } =
      this.state;
    ...
    let columns: any[] = [
      ...
      },
      {
        ...
        sortable: true,
        cell: (t: TermListing) => (
          <TermStatusBox status={t.ontologyTerm.status} />
        ),
      },
      {
        name: "Entities",
    ...
    return (
      <Fragment>
        ...
        <Grid container justify="space-between">
          ...
          <Grid item>
            ...
            >
              ...
              >
                <CloudDownload /> &nbsp; Download CSV
              </Button>
              &nbsp;
              <Button
                variant="outlined"
                color="primary"
                onClick={this.markActioned}
                disabled={!actionable}
              >
                <DoneAll /> &nbsp; Mark All As Actioned
              </Button>
            </Box>
          </Grid>
        </Grid>
        ...
        <p>
          OntoString allows all of the terms marked as <b>Needs Import</b> to be
          downloaded as a CSV file. This file can be used to inform the manual
          process of requesting that the terms are added to the correct
          ontology.
        </p>
        <p>
          When the terms no longer require attention (i.e., issues have been
          opened with the relevant ontology issue tracker), they can be marked
          as <b>actioned</b>, at which point they will no longer be displayed in
          the <b>Needs Import</b> list.
        </p>
        <p>
          Finally, OntoString will pick up the terms when they are added to the
          ontology, and display them in the <b>Current</b> list, which is the
          end of the curation workflow.
        </p>
        ...
        {context && (
          <Fragment>
            ...
            >
              <Tab
                label={`Deleted (${stats?.DELETED || "0"})`}
                value="DELETED"
              />
              <Tab
                label={`Obsolete (${stats?.OBSOLETE || "0"})`}
                value="OBSOLETE"
              />
              <Tab
                label={`Needs Import (${stats?.NEEDS_IMPORT || "0"})`}
                value="NEEDS_IMPORT"
              />
  ...
  async fetchTermListings() {
    ...
    let [res, statsRes] = await Promise.all([
      fetch(
        `${process.env.REACT_APP_APIURL}/v1/projects/${
          project.id
        }/ontology-terms?${new URLSearchParams({
      ...
      fetch(
        `${process.env.REACT_APP_APIURL}/v1/projects/${
          project.id
        }/ontology-terms-stats?${new URLSearchParams({
          ...(context ? { context: context.name! } : {}),
        })}`,
        {
          headers: { ...getAuthHeaders() },
  ...
  downloadCSV = async () => {
    ...
    let res = await fetch(
      `${process.env.REACT_APP_APIURL}/v1/projects/${project.id}/ontology-terms/export`,
      {
        method: "POST",
        body: JSON.stringify({
          status: tab,
  ...
  actionTerms = async (comment: string) => {
    ...
    let res = await fetch(
      `${process.env.REACT_APP_APIURL}/v1/projects/${project.id}/ontology-terms/action`,
      {
        method: "POST",
        body: JSON.stringify({
          status: tab,
